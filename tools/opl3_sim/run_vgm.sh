#!/bin/bash
# run_vgm.sh — _113d: reproduce un volcado de escrituras VGM (bank reg val
# espera) contra el core OPL3 y mide RMS. Uso: bash run_vgm.sh vgm01.txt
...
if [ ! -x obj_timer/Vopl3 ]; then
    rm -rf obj_timer
    verilator --cc --exe --build -j 4 -Wno-fatal --Mdir obj_timer --top-module opl3 $FILES tb_timer.cpp 2>&1 | tail -1
fi
./obj_timer/Vopl3 vgm "$1"
