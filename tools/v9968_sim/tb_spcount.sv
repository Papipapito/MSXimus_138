// ============================================================================
// tb_spcount.sv — ¿FUNCIONA LA COLISION DE SPRITES DEL V9968? (la espinita)
//
...
// ---------------------------------------------------------------------------
// tb_spcount - CUENTA DE SPRITES RENDERIZADOS (modo 1, el mundo de DQ2).
// Sintoma de placa (s027): DQ2 ya no cuelga pero NO SALEN los sprites de los
// NPC. ¿Los escondio el _187b/c (que cambio el barrido de atributos)?
// 6 sprites solidos color 15 repartidos en 3 lineas y planos 0,1,2,5,6,10
// (+ terminador): se cuentan los pixeles BLANCOS de un frame. Esperado 384
// (6 x 8x8). Cualquier numero menor = sprites perdidos.
integer i, j, blancos, blancos_prev;
logic [7:0] v;
...
initial begin
    ...
    reset_n = 1;
    repeat (48) @(posedge clk);
    $display("=== tb_spcount: sprites renderizados en modo 1 (SCREEN 2) ===");

    // SCREEN 2, backdrop negro, sprites 8x8 sin magnificar
    vdp_reg(6'd0, 8'h02);
    vdp_reg(6'd2, 8'h06);          // NT=1800
    vdp_reg(6'd3, 8'hFF);          // CT=2000
    vdp_reg(6'd4, 8'h03);          // PGT=0000
    vdp_reg(6'd5, 8'h36);          // SAT=1B00
    vdp_reg(6'd6, 8'h07);          // SPT=3800
    vdp_reg(6'd7, 8'h00);          // backdrop 0
    ...
    // SAT: 6 sprites color 15 en planos 0,1,2 (linea 50) 5,6 (80) 10 (110)
    vram_set_wr(18'h01B00);
    z80_out(2'd0, 8'd50);  z80_out(2'd0, 8'd20);  z80_out(2'd0, 8'd0); z80_out(2'd0, 8'd15);
    z80_out(2'd0, 8'd50);  z80_out(2'd0, 8'd50);  z80_out(2'd0, 8'd0); z80_out(2'd0, 8'd15);
    z80_out(2'd0, 8'd50);  z80_out(2'd0, 8'd80);  z80_out(2'd0, 8'd0); z80_out(2'd0, 8'd15);
    z80_out(2'd0, 8'd220); z80_out(2'd0, 8'd0);   z80_out(2'd0, 8'd0); z80_out(2'd0, 8'd15);
    z80_out(2'd0, 8'd220); z80_out(2'd0, 8'd0);   z80_out(2'd0, 8'd0); z80_out(2'd0, 8'd15);
    z80_out(2'd0, 8'd80);  z80_out(2'd0, 8'd30);  z80_out(2'd0, 8'd0); z80_out(2'd0, 8'd15);
    z80_out(2'd0, 8'd80);  z80_out(2'd0, 8'd60);  z80_out(2'd0, 8'd0); z80_out(2'd0, 8'd15);
    for (i = 7; i < 10; i = i + 1) begin
        z80_out(2'd0, 8'd220); z80_out(2'd0, 8'd0); z80_out(2'd0, 8'd0); z80_out(2'd0, 8'd15);
    end
    z80_out(2'd0, 8'd110); z80_out(2'd0, 8'd120); z80_out(2'd0, 8'd0); z80_out(2'd0, 8'd15);
    z80_out(2'd0, 8'd208); // terminador (plano 11)

    espera_frames(2);

    // contar blancos de UN frame completo
    for (j = 0; j < 3; j = j + 1) begin
        ...
        join
        // el pixel clock es 1/2 del clk: los conteos salen x2, se normaliza
        $display("  frame %0d: pixeles de sprite = %0d (normalizado %0d, esperado 384)",
                 j, blancos, blancos/2);
    end
    $finish;
end
